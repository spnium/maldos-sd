...
let pose1LeftStar = new Star(sideStarCoordinates[0], sideStarCoordinates[1], () => {
    return pose1LeftStar.isTouchingCoordinates(wristMiddlePoint());
}, undefined, true);
let pose1RightStar = new Star(width - sideStarCoordinates[0], sideStarCoordinates[1], () => {
    return pose1RightStar.isTouchingCoordinates(wristMiddlePoint());
}, undefined, true);
let angleMin = 150;
let angleMax = 200;
let pose1LeftElbowStar = new Star(poseCoordinates[poseLMS.LEFT_ELBOW][0], poseCoordinates[poseLMS.LEFT_ELBOW][1], () => {
    return left_arm_angle() > angleMin && left_arm_angle() < angleMax;
}, () => {
    return poseCoordinates[poseLMS.LEFT_ELBOW];
});
let pose1RightElbowStar = new Star(poseCoordinates[poseLMS.RIGHT_ELBOW][0], poseCoordinates[poseLMS.RIGHT_ELBOW][1], () => {
    return right_arm_angle() > angleMin && right_arm_angle() < angleMax;
}, () => {
    return poseCoordinates[poseLMS.RIGHT_ELBOW];
});
let wristsMiddleStar = new Star(wristMiddlePoint()[0], wristMiddlePoint()[1], () => {
    return coordinatesTouching(poseCoordinates[poseLMS.RIGHT_WRIST], poseCoordinates[poseLMS.LEFT_WRIST], [300, 200]);
}, () => {
